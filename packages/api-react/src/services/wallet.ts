...
import { CAT, DID, Farmer, NFT, Pool, WalletService, WalletType, toBech32m, VC } from '@lottery-network/api';
import type { NFTInfo, Transaction, Wallet, WalletBalance } from '@lottery-network/api';
...
export const walletApi = apiWithTag.injectEndpoints({
  endpoints: (build) => ({
    ...
    setAutoWithdrawStake: mutation(build, WalletService, 'setAutoWithdrawStake', {
      invalidatesTags: [{ type: 'AutoWithdrawStake' }],
    }),
    getAutoWithdrawStake: query(build, WalletService, 'getAutoWithdrawStake', {
      providesTags: (result) => (result ? [{ type: 'AutoWithdrawStake' }] : []),
    }),

    stakeInfo: query(build, WalletService, 'stakeInfo'),

    // TODO refactor
    sendStake: build.mutation<
      any,
      {
        walletId: number;
        stakeType: number;
        ...
        try {
          ...
          return {
            ...
            data: await new Promise(async (resolve, reject) => {
              ...
              // make transaction
              const { data: sendStakeData, error } = await fetchWithBQ({
                command: 'sendStake',
                service: WalletService,
                args: restArgs,
              });
              ...
              if (!waitForConfirmation) {
                resolve(sendStakeData);
                return;
              }

              const { transaction } = sendStakeData;
              if (!transaction) {
                reject(new Error('Transaction is not present in response'));
                return;
              }
    ...
    spendWithdrawCoins: mutation(build, WalletService, 'spendWithdrawCoins'),

    sendGuess: build.mutation<
      any,
      {
        walletId: number;
        awardAddress: string;
        ...
        try {
          ...
          return {
            ...
            data: await new Promise(async (resolve, reject) => {
              ...
              // make transaction
              const { data: sendGuessData, error } = await fetchWithBQ({
                command: 'sendGuess',
                service: WalletService,
                args: restArgs,
              });
              ...
              if (!waitForConfirmation) {
                resolve(sendGuessData);
                return;
              }

              const { transaction } = sendGuessData;
              if (!transaction) {
                reject(new Error('Transaction is not present in response'));
                return;
              }
