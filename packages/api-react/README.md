...
This library provides react hooks on the top of @lottery-network/api and uses [RTK Query](https://redux-toolkit.js.org/rtk-query/overview) under the hood.
...
- Automatically refresh queries when data changed (using events from LotteryCoin).
...
import { useGetPublicKeysQuery } from '@lottery-network/api-react';
import { Loading } from '@lottery-network/core';
...
import { store, api } from '@lottery-network/api-react';
