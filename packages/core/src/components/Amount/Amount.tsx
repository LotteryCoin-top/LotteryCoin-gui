...
import lotteryToMojo from '../../utils/lotteryToMojo';
...
export default function Amount(props: AmountProps) {
  ...
  const isLotteryCurrency = ['LOT', 'TLOT'].includes(currencyCode);
  const mojo = isLotteryCurrency ? lotteryToMojo(correctedValue) : catToMojo(correctedValue);
    ...
    <FormControl variant={variant} fullWidth={fullWidth}>
      <TextField
        ...
        InputProps={{
          ...
          inputProps: {
            decimalScale: isLotteryCurrency ? 9 : 3,
            'data-testid': dataTestid,
          },
