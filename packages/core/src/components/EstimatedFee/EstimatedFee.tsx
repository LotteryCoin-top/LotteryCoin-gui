import { useGetFeeEstimateQuery } from '@lottery-network/api-react';
...
import mojoToLotteryLocaleString from '../../utils/mojoToLotteryLocaleString';
import Fee from '../Fee';
...
export enum FeeTxType {
  walletSendLOT = 'send_lot_transaction',
...
export default function EstimatedFee(props: FeeProps) {
  ...
  const formattedEstimates: FormattedEstimate[] = useMemo(() => {
    ...
    return estList.map((estimate: number, i: number) => {
      const formattedEstimate = mojoToLotteryLocaleString(estimate, locale);
  ...
  useEffect(() => {
    if (formattedEstimates && inputType === 'dropdown') {
      ...
      if (selectedTime) {
        ...
        if (estimate) {
          const lotFee = mojoToLotteryLocaleString(estimate.estimate, 'en-US');
          setSelectedValue(estimate.formattedEstimate);
          setValue(name, lotFee);
        }
      }
    }
