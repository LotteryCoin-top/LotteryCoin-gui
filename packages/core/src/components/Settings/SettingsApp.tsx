...
import { Farming } from '@lottery-network/icons';
...
export default function SettingsApp(props: SettingsAppProps) {
  ...
  async function handleOpenFAQURL(): Promise<void> {
    try {
      ...
      await shell.openExternal('https://github.com/LotteryCoin-top/LotteryCoin/wiki/FAQ');
    } catch (error: any) {
      showError(error);
    }
  }
  ...
  async function handleOpenSendFeedbackURL(): Promise<void> {
    try {
      const { shell } = window as unknown as { shell: Shell };
      await shell.openExternal('https://feedback.lotterycoin.top/lightwallet');
    } catch (error: any) {
      showError(error);
    }
  }
    ...
    <Flex flexDirection="column" gap={3}>
      ...
      <Flex flexDirection="column" gap={1}>
        <Flex flexDirection="row" gap={1}>
          <SettingsLabel>
            <Trans>Lottery Application Version:</Trans>
          </SettingsLabel>
          {version && (
