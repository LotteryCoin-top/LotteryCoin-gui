import { useLocalStorage } from '@lottery-network/api-react';
...
export default function useGetLatestVersionFromWebsite(): UseGetLatestVersionFromWebsiteResult {
  ...
  const [latestVersionURL] = useLocalStorage<string>(
    'latestVersionURL',
    'https://download.lotterycoin.top/latest/latest.json',
  );
  ...
  const downloadUrl = downloadPath ? new URL(downloadPath, 'https://www.lotterycoin.top/').toString() : undefined;
  const releaseNotesUrl = releaseNotesPath ? new URL(releaseNotesPath, 'https://www.lotterycoin.top/').toString() : undefined;
  const blogUrl = blogPath ? new URL(blogPath, 'https://www.lotterycoin.top/').toString() : undefined;
