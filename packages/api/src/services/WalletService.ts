...
import type AutoWithdrawStake from "../@types/AutoWithdrawStake";
import type StakeValue from "../@types/StakeValue";
...
export default class Wallet extends Service {
 ...
 async sendStake(args: {
  ...
  async spendWithdrawCoins(args: { coinIds: string[]; fee: number | BigNumber; force?: boolean; }) {
    return this.command<{
      transactionIds: string[];
    }>('spend_withdraw_coins', args);
  }

 async sendGuess(args: {
    walletId: number;
    awardAddress: string;
    amount: BigNumber;
    data: string;
