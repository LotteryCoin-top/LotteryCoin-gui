import { SyncingStatus, toBech32m, WalletType } from '@lottery-network/api';
import { useSpendCATMutation, useFarmBlockMutation } from '@lottery-network/api-react';
import {
  AdvancedOptions,
  Button,
  EstimatedFee,
...
export default function WalletCATSend(props: Props) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      throw new Error(t`Cannot send lottery to coloured address. Please enter a lottery address.`);
    }

    if (address.includes('lottery_addr') || address.includes('colour_desc')) {
    ...
    if (address.slice(0, 12) === 'lottery_addr://') {
    ...
    const feeValue = lotteryToMojo(fee);
