import { TransactionType, WalletType } from '@lottery-network/api';
import type { Transaction } from '@lottery-network/api';
import { useGetWalletBalanceQuery } from '@lottery-network/api-react';
import { Color, mojoToLottery, mojoToCAT, blockHeightToTimestamp } from '@lottery-network/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(
        0,
        ([WalletType.CAT, WalletType.CRCAT].includes(walletType) ? mojoToCAT(start) : mojoToLottery(start)).toNumber(),
      ), // max 21,000,000 safe to number
      tooltip: ([WalletType.CAT, WalletType.CRCAT].includes(walletType)
        ? mojoToCAT(balance)
        : mojoToLottery(balance)
  ...
  data.forEach((item) => {
    ...
    points.push({
      ...
      tooltip: [WalletType.CAT, WalletType.CRCAT].includes(walletType)
        ? mojoToCAT(start)
        : mojoToLottery(start).toString(), // bignumber is not supported by react
    });
  });
