...
import type { OfferSummaryAssetInfo, OfferSummaryRecord } from '@lottery-network/api';
import { mojoToCAT, mojoToLottery, mojoToCATLocaleString, mojoToLotteryLocaleString } from '@lottery-network/core';
...
export function summaryStringsForNFTOffer(
  ...
  // TODO: Remove 1:1 NFT <--> LOT assumption
...
export function formatAmountForWalletType(amount: string | number, walletType: WalletType, locale?: string): string {
  if (walletType === WalletType.STANDARD_WALLET) {
    return mojoToLotteryLocaleString(amount, locale);
  }
...
export function offerAssetTypeForAssetId(assetId: string, offerSummary: OfferSummaryRecord): OfferAsset | undefined {
  ...
  if (['lot', 'tlot'].includes(assetId)) {
    assetType = OfferAsset.LOTTERY;
  } else {
    const { infos } = offerSummary;
    const info: OfferSummaryAssetInfo = infos[assetId];
...
export function offerAssetIdForAssetType(
  ...
  if (assetType === OfferAsset.LOTTERY) {
    return keys.includes('lot') ? 'lot' : undefined;
  }
...
export function getNFTPriceWithoutRoyalties(
  ...
  for (const assetType of [OfferAsset.TOKEN, OfferAsset.LOTTERY]) {
    const assetId = offerAssetIdForAssetType(assetType, summary);
    if (assetId) {
      const amountInMojos = offerAssetAmountForAssetId(assetId, summary);
      if (amountInMojos) {
        const amountInTokens = assetType === OfferAsset.LOTTERY ? mojoToLottery(amountInMojos) : mojoToCAT(amountInMojos);
