...
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToLottery, mojoToCAT } from '@lottery-network/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['lot', 'tlot'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToLottery(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['lot', 'tlot'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToLottery(takerEntries[0][1])
