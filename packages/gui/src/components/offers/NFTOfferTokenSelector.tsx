...
import type { CATToken, Wallet } from '@lottery-network/api';
import { useGetCatListQuery, useGetWalletsQuery } from '@lottery-network/api-react';
...
export default function NFTOfferTokenSelector(props: Props) {
  ...
  const [, options]: [TokenSelectOption, TokenSelectOption[]] = useMemo(() => {
    ...
    const lotteryWalletSelection = [wallets.find((wallet: Wallet) => wallet.type === WalletType.STANDARD_WALLET)].map(
      (wallet: WalletType) => ({
        walletId: wallet.id,
        walletType: wallet.type,
        name: 'Lottery',
        symbol: currencyCode,
        displayName: `Lottery (${currencyCode})`,
        disabled: false,
        tail: '',
      }),
    );
    ...
    const allOptions = [...lotteryWalletSelection, ...catOptions];
