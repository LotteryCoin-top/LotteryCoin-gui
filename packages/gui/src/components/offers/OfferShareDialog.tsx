import { OfferTradeRecord } from '@lottery-network/api';
import {
  ButtonLoading,
  CopyToClipboard,
  DialogActions,
...
const log = debug('lottery-gui:offers');
...
const testnetDummyHost = 'offers-api-sim.lotterycoin.top';
...
async function postToSpacescan(offerData: string, testnet: boolean): Promise<{ viewLink: string; offerLink: string }> {
  ...
  const requestOptions = {
    ...
    port: 443,
    path: `/api/offer/upload?coin=${testnet ? 'tlot' : 'lot'}&version=1`,
  };
  const requestHeaders = {
    'Content-Type': 'application/json',
  };
