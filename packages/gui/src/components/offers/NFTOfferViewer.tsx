import { OfferSummaryRecord, OfferTradeRecord } from '@lottery-network/api';
import { useCheckOfferValidityMutation, useGetWalletsQuery } from '@lottery-network/api-react';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] = useMemo(
    () =>
      Object.keys(summaryData).map((key) => {
        ...
        if (['lot', 'tlot'].includes(key.toLowerCase())) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.LOTTERY: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const lotMakerFee = mojoToLottery(makerFee);

    return {
      ...calculateNFTRoyalties(
        amount,
        parseFloat(lotMakerFee),
  ...
  const overrideNFTSellerAmount =
    exchangeType === NFTOfferExchangeType.TokenForNFT
      ? assetType === OfferAsset.LOTTERY
        ? lotteryToMojo(nftSaleInfo?.nftSellerNetAmount ?? 0)
