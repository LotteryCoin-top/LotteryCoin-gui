// import { useGetNFTInfoQuery } from '@lottery-network/api-react';
import { fromBech32m } from '@lottery-network/api';
import { AddressBookContext, Color, EmojiAndColorPicker, Flex, Form, TextField } from '@lottery-network/core';
...
export default function ContactEdit() {
  ...
  function handleSubmit(data) {
    ...
    filteredAddresses.forEach((entry) => {
      try {
        if (entry.address[3] === '1') {
          if (entry.address.slice(0, 3).toLowerCase() !== 'lot') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
        } else if (entry.address[4] === '1') {
          if (entry.address.slice(0, 4).toLowerCase() !== 'tlot') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
    ...
    filteredProfiles.forEach((entry) => {
      try {
        if (entry.did.slice(0, 9).toLowerCase() !== 'did:lottery:') {
          throw new Error();
