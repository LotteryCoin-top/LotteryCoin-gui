import { useGetFarmedAmountQuery } from '@lottery-network/api-react';
import {
  useCurrencyCode,
  mojoToLotteryLocaleString,
  useLocale,
  CardSimple,
  FormatLargeNumber,
  Tooltip,
...
function FarmingRewardsHistoryCards() {
  ...
  const totalLotteryFarmedCard = useMemo(() => {
    if (!data || isLoading) {
      return <CardSimple title={<Trans>Total LOT Farmed</Trans>} value="-" loading={isLoading} error={error} />;
    }
    ...
    if (data && data.farmedAmount) {
        ...
        <StyledTable>
          <tbody>
            <tr>
              ...
              <td>
                {mojoToLotteryLocaleString(data.poolRewardAmount, locale)}
                &nbsp;
                {currencyCode}
              </td>
            </tr>
            <tr>
              ...
              <td>
                {mojoToLotteryLocaleString(data.farmerRewardAmount, locale)}
                &nbsp;
                {currencyCode}
              </td>
            </tr>
            <tr>
              ...
              <td>
                {mojoToLotteryLocaleString(data.feeAmount, locale)}
                &nbsp;
                {currencyCode}
              </td>
            </tr>
      ...
      cardValue = (
        <Tooltip title={title}>
          {mojoToLotteryLocaleString(data.farmedAmount, locale)}
          &nbsp;
          {currencyCode}
        </Tooltip>
      );
    ...
    return <CardSimple title={<Trans>Total LOT Farmed</Trans>} value={cardValue} loading={isLoading} error={error} />;
