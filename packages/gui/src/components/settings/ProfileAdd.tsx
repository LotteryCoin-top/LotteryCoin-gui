...
export default function ProfileAdd() {
  ...
  const currencyCode = (useCurrencyCode() ?? 'LOT').toUpperCase();
  const isTestnet = currencyCode === 'TLOT';
  ...
  const spendableBalance = mojoToLotteryLocaleString(balance?.spendableBalance);
  const canCreateProfile = (balance?.spendableBalance ?? 0) > 0;

  function handleClick() {
    const url = `https://${isTestnet ? 'testnet11-faucet.lotterycoin.top' : 'faucet.lotterycoin.top'}/?address=${currentAddress}`;
    openExternal(url);
  }
              ...
              {!canCreateProfile && (
                <Flex flexDirection="column" gap={1}>
                  ...
                  <Link onClick={handleClick}>
                    <Trans>Get Mojos from the Lottery Faucet</Trans>
                  </Link>
                </Flex>
              )}
              <Flex flexDirection="column" gap={1}>
