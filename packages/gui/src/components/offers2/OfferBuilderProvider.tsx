import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@lottery-network/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@lottery-network/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@lottery-network/api-react';
import { catToMojo, lotteryToMojo } from '@lottery-network/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedLOT = useWatch({
    name: 'requested.lot',
  });

  const offeredLOT = useWatch({
    name: 'offered.lot',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedLOT ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('lot', lotteryToMojo(amount))),
    ...(requestedTokens ?? [])
