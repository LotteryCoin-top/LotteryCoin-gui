...
type CreateDefaultValuesParams = {
  walletType?: WalletType; // CAT or STANDARD_WALLET (LOT), indicates whether a token or CAT has a default entry
 ...
 * for creating an LOT offer, a CAT offer, or an NFT offer. LOT and CAT offers will have a default entry in
...
export default function createDefaultValues(params: CreateDefaultValuesParams | undefined = {}): OfferBuilderData {
  ...
  return {
    ...
    offered: {
      ...
      nfts,
      lot: walletType === WalletType.STANDARD_WALLET ? [{ amount: '' }] : [],
