import { Color, Flex } from '@lottery-network/core';
import { Offering, Requesting } from '@lottery-network/icons';
...
import OfferBuilderLOTSection from './OfferBuilderLOTSection';
...
export default function OfferBuilderTradeColumn(props: OfferBuilderTradeColumnProps) {
  ...
  const showLOT = !readOnly || !!lot.length;
  ...
  const mutedLOT = nfts.length || tokens.length;
  const mutedTokens = lot.length || nfts.length;
  const mutedNFTs = lot.length || tokens.length;
  ...
  return (
    <Flex flexDirection="column" gap={3}>
      ...
      >
        {showLOT && <OfferBuilderLOTSection name={`${name}.lot`} offering={offering} muted={mutedLOT} />}
