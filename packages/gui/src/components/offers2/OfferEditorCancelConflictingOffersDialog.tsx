import { ConfirmDialog, Flex } from '@lottery-network/core';
...
function getSpendableAmountUponUnlockingAssets(assetStatus: AssetStatusForOffer, assetsToBeUnlocked: PendingAsset[]) {
  ...
  for (let s = 0; s < assetsToBeUnlocked.length; s++) {
    ...
    if (lockedAsset.type === 'LOT' && assetStatus.type === 'LOT') {
      // assetId might be either 'LOT' or 'UNKNOWN'(maybe fee) but here both are treated as just a LOT spending
