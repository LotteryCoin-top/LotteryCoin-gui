import { toBech32m } from '@lottery-network/api';
import { useGetBlockQuery, useGetBlockRecordQuery } from '@lottery-network/api-react';
import {
  Back,
  Button,
  Card,
...
export default function Block() {
  ...
  const poolReward = mojoToLottery(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToLottery(calculateBaseFarmerReward(blockRecord.height));

  const lotteryFees = blockRecord.fees !== undefined ? mojoToLottery(blockRecord.fees) : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: lotteryFees ? `${lotteryFees} ${currencyCode}` : '',
  ...
  return (
    <LayoutDashboardSub>
      <Card
        title={
          <Back variant="h5">
            <Trans>Block at height {blockRecord.height} in the LotteryCoin</Trans>
          </Back>
        }
        action={
          <Flex gap={1}>
