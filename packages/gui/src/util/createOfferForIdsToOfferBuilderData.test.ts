import * as lotteryCore from '@lottery-network/core';
...
jest.mock('@lottery-network/core', () => ({
  mojoToLottery: jest.fn(),
  mojoToCAT: jest.fn(),
}));
...
describe('createOfferForIdsToOfferBuilderData', () => {
  ...
  describe('when offering LOT for CAT', () => {
    it('should return a valid offer builder data object', () => {
      ...
      lotteryCore.mojoToLottery.mockReturnValue(new BigNumber(111.555));
      lotteryCore.mojoToCAT.mockReturnValue(new BigNumber(600));
  ...
  describe('when offering a CAT for LOT', () => {
    it('should return a valid offer builder data object', () => {
      const calledLookupByWalletIdWithIds: string[] = [];
      const assetIdMapEntriesByWalletId: Record<string, AssetIdMapEntry> = {
        1: {
      ...
      lotteryCore.mojoToLottery.mockReturnValue(new BigNumber(2));
      lotteryCore.mojoToCAT.mockReturnValue(new BigNumber(1.234));
  ...
  describe('when offering LOT for an NFT', () => {
    it('should return a valid offer builder data object', () => {
      const calledLookupByWalletIdWithIds: string[] = [];
      const assetIdMapEntriesByWalletId: Record<string, AssetIdMapEntry> = {
        1: {
      ...
      lotteryCore.mojoToLottery.mockReturnValue(new BigNumber(3));
  ...
  describe('when the offered asset is for an unknown wallet type', () => {
    it('should ignore the unknown wallet assets', () => {
      ...
      lotteryCore.mojoToLottery.mockReturnValue(new BigNumber(0.5));
  ...
  describe('when lookupByWalletId throws', () => {
    ...
    it('should ignore the asset that caused the error', () => {
      ...
      lotteryCore.mojoToLottery.mockReturnValue(new BigNumber(5));
      lotteryCore.mojoToCAT.mockReturnValue(new BigNumber(1000));
