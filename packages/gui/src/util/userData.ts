...
export function getUserDataDir(): string {
  const lotteryRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(lotteryRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const lotteryRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${lotteryRootUserDataPath}`);
  app.setPath('userData', lotteryRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(lotteryRootUserDataPath, 'Local Storage', 'leveldb');
