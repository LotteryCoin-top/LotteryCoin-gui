import { toBech32m, WalletType } from '@lottery-network/api';
import { mojoToLottery, mojoToCAT } from '@lottery-network/core';
...
export default function createOfferForIdsToOfferBuilderData(
      ...
      if (asset) {
        switch (asset.walletType) {
          case WalletType.STANDARD_WALLET:
            section.lot.push({ amount: mojoToLottery(numericValue.abs()).toFixed() });
            break;
          case WalletType.CAT:
  ...
  if (fee) {
    offerBuilderData.offered.fee = [{ amount: mojoToLottery(fee).toFixed() }];
  }
