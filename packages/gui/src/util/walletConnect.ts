...
const log = initDebug('lottery-gui:walletConnect');

const availableCommands = walletConnectCommands.map((command) => `lottery_${command.command}`);
...
export async function processSessionProposal(
  ...
  try {
    ...
    const requiredNamespace = requiredNamespaces.lottery;
    if (!requiredNamespace) {
      throw new Error('Missing required lottery namespace');
    }

    const { chains, methods } = requiredNamespace;
    const chain = chains.find((item) => ['lottery:testnet', 'lottery:mainnet'].includes(item));
    if (!chain) {
    ...
    const accounts = fingerprints.map((fingerprint) => `lottery:${instance}:${fingerprint}`);
