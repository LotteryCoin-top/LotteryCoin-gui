...
import { lotteryToMojo, catToMojo } from '@lottery-network/core';
...
export default async function offerBuilderDataToOffer({
  ...
  const {
    offered: { lot: offeredLot = [], tokens: offeredTokens = [], nfts: offeredNfts = [], fee: [firstFee] = [] },
    requested: { lot: requestedLot = [], tokens: requestedTokens = [], nfts: requestedNfts = [] },
  } = data;

  const usedNFTs: string[] = [];

  const feeInMojos = firstFee ? lotteryToMojo(firstFee.amount) : new BigNumber(0);

  const walletIdsAndAmounts: Record<string, BigNumber> = {};
  const driverDict: Record<string, Driver> = {};

  if (!allowEmptyOfferColumn) {
    const hasOffer = !!offeredLot.length || !!offeredTokens.length || !!offeredNfts.length;
        ...
        if (assetId.toUpperCase() === 'LOT' || assetId.toUpperCase() === 'UNKNOWN') {
          ...
          if (idx > -1) {
            ...
            if (pendingOffers[idx].assetId.toUpperCase() !== assetId.toUpperCase()) {
              // Now we can distinguish that we have lot spending which is only LOT, only Fee or both LOT and Fee
              pendingOffers[idx].assetId = 'LOT+FEE';
            }
          } else {
            pendingOffers.push({
              type: 'LOT',
  ...
  let pendingLotOffer: AssetStatusForOffer | undefined;
  let pendingLot = new BigNumber(0);
  if (offeredLot.length > 0 || feeInMojos.gt(0)) {
  ...
  // offeredLot.length should be always 0 or 1
  const lotTasks = offeredLot.map(async (lot) => {
    const { amount } = lot;
    if (!amount || amount === '0') {
      throw new Error(t`Please enter an LOT amount`);
    }
    if (!standardWallet || !standardWalletBalance) {
      throw new Error(t`No standard wallet found`);
    }

    const mojoAmount = lotteryToMojo(amount);
    ...
    const hasEnoughTotalBalance = spendableBalance.plus(pendingLot).minus(feeInMojos).gte(mojoAmount);
    if (!hasEnoughTotalBalance) {
      throw new Error(t`Amount exceeds LOT total balance`);
    }

    if (pendingLotOffer) {
      // Assuming offeredLot.length is always less then or equal to 1
      pendingLotOffer.spendingAmount = mojoAmount.plus(feeInMojos);
      pendingLotOffer.spendableAmount = spendableBalance;
      pendingLotOffer.confirmedAmount = new BigNumber(standardWalletBalance.confirmedWalletBalance);
      const hasEnoughSpendableBalance = spendableBalance.gte(pendingLotOffer.spendingAmount);
      if (!hasEnoughSpendableBalance) {
        pendingLotOffer.status = 'conflictsWithNewOffer';
      } else {
        pendingLotOffer.status = 'alsoUsedInNewOfferWithoutConflict';
      }
    }
  });
  // Treat fee as lot spending
  if (offeredLot.length === 0 && feeInMojos.gt(0)) {
    if (!standardWallet || !standardWalletBalance) {
      throw new Error(t`No standard wallet found`);
    }
    ...
    if (!hasEnoughTotalBalance) {
      throw new Error(t`Fee exceeds LOT total balance`);
    }
    if (pendingLotOffer) {
      pendingLotOffer.spendingAmount = feeInMojos;
      pendingLotOffer.spendableAmount = spendableBalance;
      pendingLotOffer.confirmedAmount = new BigNumber(standardWalletBalance.confirmedWalletBalance);
      const hasEnoughSpendableBalance = spendableBalance.gte(pendingLotOffer.spendingAmount);
      if (!hasEnoughSpendableBalance) {
  ...
  await Promise.all([...lotTasks, ...tokenTasks, ...nftTasks]);

  // requested
  requestedLot.forEach((lot) => {
    const { amount } = lot;
    ...
    walletIdsAndAmounts[wallet.id] = lotteryToMojo(amount);
  });
  ...
  const prepareNftTasks = requestedNfts.map(async ({ nftId }) => {
    ...
    if (driver) {
      ...
      if (considerNftRoyalty && pendingLotOffer) {
        ...
        if (royaltyPercentageStr) {
          ...
          const spendingLot = pendingLotOffer.spendingAmount.minus(feeInMojos);
          const newSpendingLot = spendingLot.multipliedBy(royaltyMultiplier).plus(feeInMojos);
          pendingLotOffer.spendingAmount = newSpendingLot;

          const hasEnoughSpendableBalance = pendingLotOffer.spendableAmount.gte(pendingLotOffer.spendingAmount);
          if (!hasEnoughSpendableBalance) {
            pendingLotOffer.status = 'conflictsWithNewOffer';
          } else {
            pendingLotOffer.status = 'alsoUsedInNewOfferWithoutConflict';
