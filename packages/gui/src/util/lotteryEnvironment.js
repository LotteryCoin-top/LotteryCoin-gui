...
const PY_DIST_EXECUTABLE = 'lottery';
const PY_DIST_EXEC_ARGS = Object.freeze(['start', 'daemon', '--skip-keyring']);

const PY_DEV_EXECUTABLE = `../../../venv/${process.platform === 'win32' ? 'Scripts/lottery.exe' : 'bin/lottery'}`;
...
const getLotteryVersion = () => {
  let version = null;
  const exePath = getExecutablePath('lottery');
  // first see if we can get a lottery exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or lottery is on the path
    try {
      version = childProcess
...
const startLotteryDaemon = () => {
  ...
  if (!pyProc) {
    throw new Error('Failed to start lottery daemon');
  }
